...
double **matrix_chain(double ***mat_ptr, int *p, int **s, int i, int j)
{
	...
	if (i < j) {
		k = s[i][j];
		return matmul_two(*mat_ptr,matrix_chain(mat_ptr,p,s,i,k),matrix_chain(mat_ptr,p,s,k+1,j), \
			      p[i],p[k+1],p[j+1]);
	}
	else
		return *(mat_ptr + i);
...
double **matrix_chain(double ***mat_ptr, int *p, int **s, int i, int j)
{
	...
	if (i == j) {
		...
		k = s[i][j];
		double **mid_result;
		mid_result = malloc(p[i] * sizeof(double *));
		for (g = 0; g< p[i]; g++) {
			mid_result[g] = malloc(p[j+1] * sizeof(double));
		}
		mid_result = matmul_two(mid_result, matrix_chain(mat_ptr,p,s,i,k), matrix_chain(mat_ptr,p,s,k+1,j), \
					p[i],p[k+1],p[j+1]);
		return mid_result;
	}
}
