...
sub partialBusStat {
  ...
  if ( $device =~ /BUS/i or $name =~ /BUS/i ) {
    ...
    printf "%12s %g MB/s",$name,$freq*$nPackets*$size/($nCycles);
    print "\n";
    print "    : ";
  }
}
...
sub partialMemStat {
  ...
  if ($cf->getResultField("${name[1]}","readHit")==0 && $cf->getResultField("${name[1]}","readMiss")==0) {
      partialMemStat("lowerLevel", $name[0], $cpuID);
      return;
  }

  printf "%12s", $orig;
  if ($cf->getResultField("${name[1]}","readMiss")) {
    printf " %3.6f ",$cf->getResultField("${name[1]}_occ","v");
...
sub showStatReport {
  ...
  # BusyCPU
  printf " %6.6f ", 100*$totCycles/$nCycles;
  # CommitInst
...
sub tradCPUStats {
  ...
  for(my $i=0;$i<$nCPUs;$i++) {
    ...
    printf " %3d  %1.3f  %1.3f",$i,$nInst/$clockTicks, $clockTicks/$nInst;
